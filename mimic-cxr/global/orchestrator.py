...
def project_to_dim(X: np.ndarray, d: int) -> np.ndarray:
    ...
    d: target dim
    규칙:
      - PCA는 n_components <= min(n_samples, D, d) 로만 수행
      - PCA 불가/의미 없으면 단순 trim/pad
      - 최종적으로 (n_samples, d) 반환
    """
    if X.size == 0:
    ...
    # 유의미한 PCA (최소 2차원)만 수행
    if n_comp >= 2 and D > 1:
        from sklearn.decomposition import PCA
        pca = PCA(n_components=n_comp, random_state=SEED)
        Y = pca.fit_transform(X).astype(np.float32)
    else:
        # PCA가 의미없으면: 우선 D와 d 중 작은 쪽으로 잘라냄
        Y = X.astype(np.float32)
        Y = Y[:, :min(D, d)]

    # 필요시 zero-pad / 추가 trim
    if Y.shape[1] < d:
...
def _pick_num_heads(d_model: int) -> int:
    """
    MultiheadAttention의 제약: embed_dim % num_heads == 0
    d_model의 약수 중 적절한 값을 골라 반환 (최대 8, 4, 2, 1 순으로 시도)
    """
    for h in [16, 8, 4, 2]:
        if d_model % h == 0:
            return h
...
def build_global_vectors(img_cent: np.ndarray, txt_cent: np.ndarray, d_model: int) -> Dict[str, np.ndarray]:
    """
    Cross-Attention 기반 Z 생성:
      1) img_cent, txt_cent를 d_model로 투영(project_to_dim) 후 L2 정규화
      2) MultiheadAttention으로 img→txt, txt→img soft alignment 계산
      3) 어텐션 확률을 유사도로 하여 헝가리안 매칭(1:1 페어)
      4) Z_mm = 0.5 * (ctx_img2txt[i] + ctx_txt2img[j]) (페어마다)
         Z_img2txt = ctx_img2txt (페어 i 인덱스 순)
         Z_txt2img = ctx_txt2img (페어 j 인덱스 순)
    """
    ...
    # 1) d_model로 투영 + L2 정규화
    img_proj = project_to_dim(img_cent, d_model)   # (K_img, d_model)
    txt_proj = project_to_dim(txt_cent, d_model)   # (K_txt, d_model)
    img_proj = _l2norm_np(img_proj)
    txt_proj = _l2norm_np(txt_proj)

    Ki, Kt = img_proj.shape[0], txt_proj.shape[0]
    if Ki == 0 or Kt == 0:
        return {"Z_mm": np.zeros((0, d_model), np.float32),
                "Z_img2txt": np.zeros((0, d_model), np.float32),
                "Z_txt2img": np.zeros((0, d_model), np.float32)}

    # to torch (seq_len, batch=1, d_model)
    dev = torch.device("cuda" if torch.cuda.is_available() else "cpu") # 어텐션은 CPU로도 충분히 빠름 (필요시 cuda로 바꿔도 됨)

    Qi = torch.from_numpy(img_proj).to(dev).float().unsqueeze(1)  # (Ki, 1, d_model)
    Kt_ = torch.from_numpy(txt_proj).to(dev).float().unsqueeze(1) # (Kt, 1, d_model)
    Qt = Kt_.clone()
    Ki_ = Qi.clone()

    # 2) MultiheadAttention (양방향)
    h = _pick_num_heads(d_model)
    attn_img2txt = nn.MultiheadAttention(embed_dim=d_model, num_heads=h, dropout=0.0, batch_first=False, device=dev)
    attn_txt2img = nn.MultiheadAttention(embed_dim=d_model, num_heads=h, dropout=0.0, batch_first=False, device=dev)
    attn_img2txt.eval(); attn_txt2img.eval()

    with torch.no_grad():
        # img → txt : Q=img, K=txt, V=txt
        ctx_i2t, w_i2t = attn_img2txt(Qi, Kt_, Kt_, need_weights=True)  # ctx:(Ki,1,d), w:(1,Ki,Kt)
        # txt → img : Q=txt, K=img, V=img
        ctx_t2i, w_t2i = attn_txt2img(Qt, Ki_, Ki_, need_weights=True)  # ctx:(Kt,1,d), w:(1,Kt,Ki)

    # 3) 매칭 점수 행렬 (img×txt). 어텐션 확률을 유사도로 사용
    S = w_i2t.squeeze(0).cpu().numpy().astype(np.float32)  # (Ki, Kt)

    # 헝가리안으로 1:1 페어 추출
    pairs = hungarian_pairs(S)  # [(i, j)]
    if len(pairs) == 0:
        return {"Z_mm": np.zeros((0, d_model), np.float32),
                "Z_img2txt": np.zeros((0, d_model), np.float32),
                "Z_txt2img": np.zeros((0, d_model), np.float32)}

    # 4) 페어 순서대로 컨텍스트/원본 추출
    # 이후 컨텍스트들도 넘파이 변환 전 detach
    ctx_i2t_np = ctx_i2t.squeeze(1).detach().cpu().numpy().astype(np.float32)
    ctx_t2i_np = ctx_t2i.squeeze(1).detach().cpu().numpy().astype(np.float32)

    # 선택된 인덱스만 정렬해서 Z 구성
    img_idx = [i for i, _ in pairs]
    txt_idx = [j for _, j in pairs]

    Zi = ctx_i2t_np[img_idx, :]  # (K*, d)
    Zt = ctx_t2i_np[txt_idx, :]  # (K*, d)

    # L2 정규화
    Zi = _l2norm_np(Zi)
    Zt = _l2norm_np(Zt)

    Z_mm      = 0.5 * (Zi + Zt)     # 페어별 멀티모달 전역 벡터
    Z_img2txt = Zi                  # 이미지→텍스트 컨텍스트(이미지 전용 학생에게 텍스트 프록시 역할)
    Z_txt2img = Zt                  # 텍스트→이미지 컨텍스트(텍스트 전용 학생에게 이미지 프록시 역할)

    # 페어링 로그 저장 (어텐션 확률)
    with open(GLOBAL_DIR / "cluster_pairing.csv", "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["img_cluster_idx", "txt_cluster_idx", "attn_prob"])
        for (i, j) in pairs:
