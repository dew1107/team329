...
# Load the datasets
train_captions_df = pd.read_csv('../../valid_captions.csv') # data type 에 따라 train/valid/test 로 변경
train_concepts_df = pd.read_csv('../../valid_concepts.csv')
train_concepts_manual_df = pd.read_csv('../../valid_concepts_manual.csv')
...
sample_size = min(1000, num_total_ids)
...
def save_client_data(client_id, data_type, df):
    ...
    file_path = os.path.join(dir_path, f'client_{client_id}_val_{data_type}.csv')
